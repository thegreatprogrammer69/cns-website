...
import { BadgeCheck, Drama, Puzzle, Sparkles, Timer, ToyBrick } from "lucide-react";
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-8">
        ...
        <section className="kids-card grid gap-6 overflow-hidden p-6 md:grid-cols-[1.15fr_1fr] md:p-8">
          <div>
            ...
            <h1 className="mt-4 text-3xl leading-tight font-bold text-slate-900 md:text-5xl">
              Развивающий центр с тёплой атмосферой, где ребёнок учится через игру
            </h1>
            <p className="mt-4 max-w-xl text-slate-700">
              Занятия по 1,5 часа: практика, перерыв, закрепление и рефлексия. Без давления, с вниманием к каждому ребёнку и понятным результатом каждый месяц.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <Link href="/prices" className="rounded-full bg-sky-500 px-5 py-3 text-sm font-semibold text-white transition hover:bg-sky-600">
                Записаться на пробное
              </Link>
              <Link href="/schedule" className="rounded-full border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-800 transition hover:bg-slate-50">
          ...
          <div className="relative flex items-center justify-center">
            ...
            <Image src="/kids-hero.svg" alt="Иллюстрация детского центра" width={560} height={380} className="relative z-10 w-full max-w-md" priority />
          </div>
        </section>
      </AnimatedSection>

      <AnimatedSection delay={0.04}>
        <Section title="Наши направления" subtitle="Минимум теории, максимум практики и живого участия ребёнка.">
          <div className="grid gap-4 md:grid-cols-2">
            {directions.map((direction) => {
              const Icon = directionIcons[direction.slug as keyof typeof directionIcons] ?? Sparkles;
              return (
                <article key={direction.slug} className="rounded-2xl border border-slate-200 bg-gradient-to-b from-white to-slate-50 p-4 transition hover:-translate-y-0.5 hover:shadow-md">
                  <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-sky-100 text-sky-700">
                    <Icon size={18} />
                  </span>
                  <h3 className="mt-3 text-xl font-semibold text-slate-900">{direction.title}</h3>
                  <p className="mt-2 text-sm text-slate-600">{direction.shortText}</p>
                  <Link href={`/directions/${direction.slug}`} className="mt-3 inline-flex text-sm font-medium text-sky-600">
                    Подробнее →
                  </Link>
                </article>
              );
      ...
      <AnimatedSection delay={0.08}>
        <Section title="Почему родители выбирают нас" subtitle="Удобное пространство, продуманный формат и дружелюбная атмосфера.">
          <div className="grid gap-4 md:grid-cols-2">
            {[
      ...
      <AnimatedSection delay={0.12}>
        <Section title="Формат занятий 1,5 часа" subtitle="Практика → перерыв → закрепление → рефлексия.">
          <div className="grid gap-3 md:grid-cols-4">
            {["Практика", "Перерыв", "Закрепление", "Рефлексия"].map((step, index) => (
      ...
      <AnimatedSection delay={0.16}>
        <Section title="Что вы увидите в результате занятий" subtitle="Каждый месяц ребёнок показывает понятный итог.">
          <div className="grid gap-4 md:grid-cols-[1.2fr_1fr]">
            <div className="rounded-2xl border border-slate-200 bg-white p-4 text-slate-700">
              <p>
                Сценка, портфолио, видео, поделка, проект или мини-выступление. Мероприятие проходит раз в месяц и длится до 1,5 часов.
              </p>
      ...
      <AnimatedSection delay={0.2}>
        <Section title="График занятий" subtitle="Удобное расписание, которое легко совмещать с учебой и работой.">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {plans.map((plan) => (
              <article key={plan.slug} className="rounded-2xl border border-slate-200 bg-white p-4">
                {plan.isPopular ? (
                  <span className="inline-flex rounded-full bg-rose-100 px-2 py-1 text-xs font-semibold text-rose-700">Популярный</span>
                ) : null}
